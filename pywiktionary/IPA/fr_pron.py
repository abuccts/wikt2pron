...
def to_IPA(text, pos):
    def repl1(match):
        ...
        if vowel in remove_diaeresis_from_vowel.keys():
            return "gu" + remove_diaeresis_from_vowel[vowel]
        return "g" + vowel
    def repl2(match):
        v1, v2, mn, c = \
    ...
    text = re.sub("(" + cons_c + r")\1e([⁀‿])", r"\1\2", text) # TO BE FIXED
    ...
    text = re.sub(r"⁀e([mn])\1(" + vowel_c + ")", r"⁀en_\1\2", text) # emmener, ennui # TO BE FIXED
    text = re.sub("⁀(h?)[eæ](" + cons_c + r")\2", "⁀\1é\2", text) # effacer, essui, errer, henné # TO BE FIXED
    text = re.sub("[eæ](" + cons_c + r")\1", r"è\1", text) # mett(r)ons, etc. # TO BE FIXED
